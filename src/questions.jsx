import hinh1 from './assets/hinh1.png';
import hinh2 from './assets/hinh2.png';
import hinh3 from './assets/hinh3.png';
import hinh4 from './assets/hinh4.png';
import hinh5 from './assets/hinh5.png';
import hinh6 from './assets/hinh6.png';
import hinh7 from './assets/hinh7.png';
import hinh8 from './assets/hinh8.png';
import hinh9 from './assets/hinh9.png';
import hinh10 from './assets/hinh10.png';

// Import hình ảnh cho từng đáp án
import hinh1A from './assets/hinh1A.png';
import hinh1B from './assets/hinh1B.jpg';
import hinh1C from './assets/hinh1C.jpg';
import hinh1D from './assets/hinh1D.jpg';

import hinh2A from './assets/hinh2A.png';
import hinh2B from './assets/hinh2B.png';
import hinh2C from './assets/hinh2C.jpg';
import hinh2D from './assets/hinh2D.jpg';

import hinh3A from './assets/hinh3A.jpg';
import hinh3B from './assets/hinh3B.jpg';
import hinh3C from './assets/hinh3C.jpg';
import hinh3D from './assets/hinh3D.jpg';

import hinh4A from './assets/hinh4A.jpg';
import hinh4B from './assets/hinh4B.png';
import hinh4C from './assets/hinh4C.png';
import hinh4D from './assets/hinh4D.jpg';

import hinh5A from './assets/hinh5A.png';
import hinh5B from './assets/hinh5B.png';
import hinh5C from './assets/hinh5C.png';
import hinh5D from './assets/hinh5D.png';

import hinh6A from './assets/hinh6A.jpg';
import hinh6B from './assets/hinh6B.jpg';
import hinh6C from './assets/hinh6C.jpg';
import hinh6D from './assets/hinh6D.jpg';

export default [
  {
    id: 'q1',
    text: 'Câu 1: Mắt của bạn có màu gì?',
    image: hinh1,
    answers: [
      {
        text: 'A. Nâu sáng pha ánh đen.',
        image: hinh1A
      },
      {
        text: 'B. Nâu đen hoặc đen.',
        image: hinh1B
      },
      {
        text: 'C. Nâu pha ánh đỏ đồng.',
        image: hinh1C
      },
      {
        text: 'D. Nâu pha ánh đỏ.',
        image: hinh1D
      },
    ],
  },
  {
    id: 'q2',
    text: 'Câu 2: Màu da của bạn là gì?',
    image: hinh2,
    answers: [
      {
        text: 'A. Da trắng, ánh hồng.',
        image: hinh2A
      },
      {
        text: 'B. Da trắng, ánh vàng.',
        image: hinh2B
      },
      {
        text: 'C. Da ngăm, oliu.',
        image: hinh2C
      },
      {
        text: 'D. Da nâu.',
        image: hinh2D
      }
    ],
  },
  {
    id: 'q3',
    text: 'Câu 3: Màu tĩnh mạch ở cổ tay bạn là gì?',
    image: hinh3,
    answers: [
      {
        text: 'A. Mạch máu có màu xanh lá nhạt',
        image: hinh3A
      },
      {
        text: 'B. Mạch máu có màu xanh dương pha tím',
        image: hinh3B
      },
      {
        text: 'C. Mạch máu có màu xanh lá pha tím',
        image: hinh3C
      },
      {
        text: 'D. Mạch máu có màu xanh dương hoặc tím',
        image: hinh3D
      }
    ],
  },
  {
    id: 'q4',
    text: 'Câu 4: Da bạn khi tiếp xúc với nắng thay đổi như thế nào?',
    image: hinh4,
    answers: [
      {
        text: 'A. Da dễ bắt nắng nhưng nhanh chóng hồi phục.',
        image: hinh4A
      },
      {
        text: 'B. Da dễ bắt nắng và khó hồi phục.',
        image: hinh4B
      },
      {
        text: 'C. Da ửng đỏ nhưng khó cháy nắng.',
        image: hinh4C
      },
      {
        text: 'D. Da ửng đỏ và có cháy nắng.',
        image: hinh4D
      }
    ],
  },
  {
    id: 'q5',
    text: 'Câu 5: Màu son nào giúp bạn trông rạng rỡ nhất?',
    image: hinh5,
    answers: [
      {
        text: 'A. MÀU SAN HÔ, HỒNG ĐÀO, ĐỎ CAM...',
        image: hinh5A
      },
      {
        text: 'B. HỒNG PHẤN, HỒNG CÁNH SEN, BERRY NHẠT...',
        image: hinh5B
      },
      {
        text: 'C. ĐỎ GẠCH, ĐỎ CAM TRẦM, NÂU ĐỎ, CAM ĐẤT...',
        image: hinh5C
      },
      {
        text: 'D. ĐỎ RUBY, ĐỎ MẬN, ĐỎ RƯỢU VANG...',
        image: hinh5D
      }
    ],
  },
  {
    id: 'q6',
    text: 'Câu 6: Bạn thấy da mình được tôn lên nhất khi đeo trang sức loại nào?',
    image: hinh9,
    answers: [
      {
        text: 'A. Vàng.',
        image: hinh6A
      },
      {
        text: 'B. Bạc.',
        image: hinh6B
      },
      {
        text: 'C. Vàng hồng.',
        image: hinh6C
      },
      {
        text: 'D. Lấp lánh.',
        image: hinh6D
      }
    ],
  },
];
